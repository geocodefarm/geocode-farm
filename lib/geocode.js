...
module.exports = (rows) => {
  ...
  async.map(rows, (row, done) => {
    adresse(row.address, row.postal_code, row.city, (err, data) => {
      if (err) {
        logger.error(`[api-adresse] Api failed for ${data.address}`);
      }

      if (! data.latitude || ! data.longitude) {
        logger.error(`[api-adresse] Unable to geocode ${data.address}`);
      }
