...
let db = require('./db');
let cityHall = require('./city-hall');
...
module.exports = (rows) => {
  ...
  async.map(rows, (row, done) => {
    cityHall(row.postal_code, row.city, (err, data) => {
      if (err) {
        logger.error(`[insee] Code failed for ${row.postal_code} ${row.city}`);
      }

      if (! data.latitude || ! data.longitude) {
        logger.error(`[mairie] Unable to geocode ${row.postal_code} ${row.city}`);
      }
